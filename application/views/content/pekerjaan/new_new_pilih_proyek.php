    ...
    <div class="card-body table-responsive">
      <table id="example1" class="table table-bordered table-striped responsif">
        ...
        <tr>
          ...
          <td style="text-align: center;"><?php echo number_format($value->total_bobot, 3,',','.'); ?></td>
          <td style="text-align: center;">
            <?php
              if ($value->status == 1) {
                  echo '<span class="badge badge-warning">Request Pengawas</span>';
                }
                elseif ($value->status == 2) {
                  echo '<span class="badge badge-success">Telah Disetujui PPK</span>';
                }
            ?>
          ...
          <?php if($this->session->userdata('posisi') != "PPK"){ ?>
            <form method="post" id="form" action="<?php echo base_url('pekerjaan/new_new_cek_validasi_minggu')?>">
          ...
          <?php if($this->session->userdata('posisi') == "PPK"){  ?>
            <form method="post" id="form" action="<?php echo base_url('pekerjaan/new_new_lihat_validasi_minggu')?>">
                ...
                <!-- <input type="text" name="proyekk"> -->
                <button type="submit" id="" class="btn btn-sm btn-primary"><span><i class=""></i> </span><p id="">Lihat</p></button>
            </form>
          <?php } ?>
          </td>
        </tr>
