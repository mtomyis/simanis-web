...
class M_api extends CI_Model {
	...
	function pekerjaan($user_apa)
	{
        ...
        foreach ($q->result() as $row) {
        	...
        	$qu =  "SELECT SUM(bobot_detail) as bobotsdminggu, tanggalakhir FROM `new_bobot_uraian_kerja`JOIN new_pekerjaan ON new_pekerjaan.id = new_bobot_uraian_kerja.fk_id_new_pekerjaan join new_pekerjaan_detail on  new_pekerjaan_detail.proyek = new_pekerjaan.proyek WHERE new_pekerjaan.proyek= '{$proyeku}'";
			...
			if ($query->num_rows() > 0) {
				foreach ($query->result() as $rowk) {
					...
					$a[] =array("proyek"=>$proyeku,"total_bobot"=>number_format($rowk->bobotsdminggu, 3,',','.'), "tersisa"=>$tersisa,"idproyek"=>$id_proyek);
				}
			}
        }
        return $a;
    ...
    public function kirim_laporan($proyek, $idminggu, $ttd)
	{
		$query = "  UPDATE new_minggu set status_pengawas = 2, status_ppk = 1, ttd_pengawas = '$ttd', `tanggal_laporan`= NOW() where proyek = '{$proyek}' and id = {$idminggu}
		";
		$daftar =  $this->db->query($query);
		return $daftar;
	}
	
	public function kirim_laporan_revisi($proyek, $idminggu, $ttd)
	{
	    $ttdpeng= "";

        $pro = "SELECT ttd_pengawas FROM `new_minggu` WHERE id = '$idminggu' ";
		...
		$query = "  UPDATE new_minggu set status_pengawas = 2, status_ppk = 4, ttd_pengawas = '$ttd', `tanggal_laporan`= NOW() where proyek = '{$proyek}' and id = {$idminggu}
		";
		$daftar =  $this->db->query($query);
		return $daftar;
	}
    ...
    public function kirim_laporan_pdf($idminggu, $namaPdf)
    {
        ...
        $pro = "SELECT pdf FROM `new_minggu` WHERE id = '$idminggu' ";
        $sanaa =  $this->db->query($pro);
          if ($sanaa->num_rows() > 0) {
            foreach ($sanaa->result() as $row) {
                $nama_pdf = $row->pdf;
    ...
    {
        $query = "SELECT *FROM `new_minggu` WHERE id = {$mingguke}
        ";
        $daftar =  $this->db->query($query)->row();
