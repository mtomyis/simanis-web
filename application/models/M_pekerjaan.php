...
{	
    ...
    // pilihannya ada 2 ini dan yang bawah ini
    {
        ...
        $query = "  SELECT section FROM new_pekerjaan where proyek = '{$proyek}' GROUP BY section ORDER BY id
        ";
    ...
    public function kosongkan($proyek)
    {
        ...
        $query = "  DELETE FROM new_minggu where proyek = '{$proyek}'
        ";
        $daftar2 =  $this->db->query($query);
        ...
        $query = "  DELETE FROM dokumentasi where proyek = '{$proyek}'
        ";
        $daftar =  $this->db->query($query);
    ...
    public function simpandatabobotacuan()
    {
        ...
        if ($datamingguke == "minggu ke- 1") {
            ...
            $idm = $idminggu-1;
            $sql = "SELECT * FROM new_minggu where id = $idm";
            $query =  $this->db->query($sql);
            if ($query->num_rows() > 0) {
                foreach ($query->result() as $rowy) {
                    $bobotkomulatif = $rowy->bobot_total_acuan_komulatif+$databobotacuan;
                    $bobotacuan = $databobotacuan;
                    // update datanya
                }
            }
        ...
        $querysql="UPDATE `new_minggu` SET  `bobot_total_acuan`= '{$bobotacuan}',  `bobot_total_acuan_komulatif`= '{$bobotkomulatif}' WHERE id = $idminggu";
    ...
    {
        $query = "  SELECT minggu FROM `new_minggu` WHERE id = {$mingguke} AND proyek= '{$proyek}'
        ";
        $daftar =  $this->db->query($query)->row();
    ...
    {
        $query = "  SELECT*FROM new_minggu where proyek = '$proyek'
        ";
        $daftar =  $this->db->query($query)->result();
    ...
    {
        $query = "SELECT new_minggu.id as id_minggu, new_minggu.minggu as minggu, new_pekerjaan_detail.proyek as nama_proyek, new_minggu.tgl_awal as tgl_pertama, new_minggu.tgl_akhir as tgl_kedua, new_minggu.bobot_total as total_bobot,new_minggu.{$status} as status FROM `new_minggu` JOIN new_pekerjaan_detail ON new_pekerjaan_detail.proyek = new_minggu.proyek WHERE new_minggu.{$status} = 1 AND new_pekerjaan_detail.{$fk}= {$idfk}
        ";
        $daftar =  $this->db->query($query)->result();
    ...
    public function new_new_kirim_laporan($proyek, $idminggu)
     {
        $query = "  UPDATE new_minggu set status_ppk = 1 where proyek = '{$proyek}' and id = {$idminggu}
        ";
        $daftar =  $this->db->query($query);
     ...
     {
        $query = "  UPDATE new_minggu set status_ppk = 2, status_kpa = 1, status_ppspm = 1, status_kasubdit = 1 
    ...
    {
        $query = "  SELECT bobot_total_acuan_komulatif FROM new_minggu where proyek = '{$proyek}'
        ";
        $daftar =  $this->db->query($query)->result_array();
    ...
    {
        $query = "  SELECT format(bobot_total, 3) as bobot_total FROM new_minggu where proyek = '{$proyek}'
        ";
        $daftar =  $this->db->query($query)->result_array();
    ...
    {
        $query = "  SELECT minggu FROM new_minggu where proyek = '{$proyek}'
        ";
        $daftar =  $this->db->query($query)->result_array();
